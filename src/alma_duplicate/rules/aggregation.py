...
def aggregate_continuum(context, criteria, *, supported=True):
    results = {}
    ...
    if not supported or not isinstance(context.evidence, ArchiveContextEvidence):
        truth = Truth.UNKNOWN
        reasons.append("BRANCH_SCOPE_UNSUPPORTED")
    elif not context.evidence.row_link.is_linked:
        truth = Truth.UNKNOWN
    ...
    return BranchAssessment("CONTINUUM", context.context_id, status, truth,
                            CONTINUUM_CRITERIA, tuple(reasons))
